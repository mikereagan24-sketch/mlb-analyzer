...
| **wOBA snapshot regime split** | `SELECT snapshot_date, COUNT(*), MAX(sample_size) FROM woba_data_snapshot WHERE data_key='bat-act-rhp' GROUP BY 1` | a snapshot-bound sweep pooling the FIVE actuals regimes — two-year unqualified to 2026-07-01, qualifier-restricted to 07-30, **single-season 08-03 → 09-21**, corrected from 09-22, and **2026-09-23** the 0.210 batter wOBA floor retired then replaced by a sample floor at MIN_PA (an INGEST boundary, classified by CONTENT -- `MIN(snapshot_date) WHERE data_key LIKE 'bat-act-%' AND woba < 0.210` -- because `upload_log` holds only a count and 818 post-change was BELOW the 823 an earlier regime had already reached; the count rule fired on one key and not the other for the same upload). `parameter-sweep.js` and `calibration-sweep.js` bind each game to its own date's snapshot, so any window crossing those dates mixes them; the 2026-08-21 W_PROJ/W_ACT sweep spanned three. Classified by row count and max sample, not by a marker column or a remembered date — see `docs/actuals-single-season-rebaseline-2026-09-22.md` |
...
| **A/B that swaps a module export** | `node scripts/test-woba-park-source.js` | a consumer that DESTRUCTURES at require time never sees the swap, so the A/B compares two identical runs and reports "nothing moved" — which reads as "safe to ship" |
| **Bullpen park-neutralization** | `node scripts/test-bullpen-neutral.js` | the schema layer growing a copy of the neutralization lookup, or the resolver becoming fatal — a neutralization failure must degrade to raw actuals, not take the pool down |
...
| **The whole suite, against its baseline** | `npm test` | a suite nobody runs. `scripts/run-tests.js` has globbed every `scripts/test-*.js` and diffed `test-baseline.json` since 2026-08-30, and **nothing invoked it** — no npm script, no CI, no cron, no row here — so `test-emit-time-provenance` sat red for 13 days while the mechanism that would have called it a REGRESSION on day one sat unused. It caps every child at 1536MB per the 2GB rule below; exit 1 on any divergence. **Run it under Node 20** — `npm` takes `node` from PATH, which is Node 24 here, and better-sqlite3's binding is ABI 115; the runner probes the binding first and aborts with **exit 2** ("could not run") rather than reporting the 50 phantom regressions that an ABI mismatch produces |
