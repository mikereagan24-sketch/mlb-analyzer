...
// On Render, use /data for persistent disk. Locally use ./data
...
db.exec(`
  CREATE TABLE IF NOT EXISTS woba_data (
    ...
    data_key    TEXT NOT NULL,       -- e.g. 'bat-proj-lhp'
  ...
  CREATE TABLE IF NOT EXISTS bet_signals (
    ...
    game_date    TEXT NOT NULL,
    game_id      TEXT NOT NULL,
    signal_type  TEXT NOT NULL,   -- 'ML' or 'Total'
    signal_side  TEXT NOT NULL,   -- 'away','home','over','under'
    signal_label TEXT NOT NULL,   -- 'Value' or 'Lean'
    category     TEXT NOT NULL,   -- 'val-fav','lean-dog', etc.
    market_line  INTEGER,         -- market ML or total line
    model_line   INTEGER,         -- model ML or model total*10 for totals
    edge_pct     REAL,            -- implied prob edge
  ...
  CREATE TABLE IF NOT EXISTS cron_log (
    id         INTEGER PRIMARY KEY AUTOINCREMENT,
    job_type   TEXT NOT NULL,   -- 'lineups' or 'scores'
...
const q = {
  ...
  wobaKeySummary: db.prepare(`
    SELECT data_key, COUNT(*) as row_count, MAX(uploaded_at) as uploaded_at
    FROM woba_data GROUP BY data_key
  `),
  ...
  upsertGame: db.prepare(`
    ...
    ON CONFLICT(game_date, game_id) DO UPDATE SET
      ...
      model_away_ml = excluded.model_away_ml,
      model_home_ml = excluded.model_home_ml,
  ...
  getGamesByDate: db.prepare(`SELECT * FROM game_log WHERE game_date = ? ORDER BY game_id`),
  ...
  insertSignal: db.prepare(`
    INSERT INTO bet_signals (
      game_log_id, game_date, game_id,
      signal_type, signal_side, signal_label, category,
      market_line, model_line, edge_pct, outcome, pnl
    ) VALUES (
      @game_log_id, @game_date, @game_id,
      @signal_type, @signal_side, @signal_label, @category,
      @market_line, @model_line, @edge_pct, @outcome, @pnl
  ...
  getSignalsByDateRange: db.prepare(`
  ...
  // cron_log
  logCron: db.prepare(`
    INSERT INTO cron_log (job_type, run_date, status, message, games_updated)
    VALUES (?, ?, ?, ?, ?)
  `),
...
// Batch upsert woba rows inside a transaction
