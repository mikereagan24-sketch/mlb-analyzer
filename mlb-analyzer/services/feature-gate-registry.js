...
const GATES = [
  ...
  { id: 'spread_edge_display_enabled', key: null, on_expected: false,
    criterion: 'OFF 2026-09-11. The empirical-spread pp figures are not shown on the '
             ...
             + 'measure and a monotone recalibration passed it while being worse on '
             + 'everything else; see AMENDMENT in the note). A recalibrated or '
             ...
             + '  (4) AUC AT LEAST the market\'s.\n'
             + 'Clause (4) is the load-bearing one: it is the only clause a monotone '
             + 'recalibration cannot game, because AUC depends solely on ranking and a '
             + 'monotone map cannot reorder. Clauses (2) and (3) stop a forecast that '
             + 'buys calibration with sharpness -- a constant base-rate predictor is '
             + 'perfectly calibrated and worthless. Beating the RAW engine is not the '
             + 'bar; the raw engine is the thing that failed. Re-run: the walk-forward '
             + 'harness described in the founding measurement below.',
    criterion_type: 'calibration', precondition: null,
    ...
    note: 'FOUNDING MEASUREMENT (this is what decided it). Walk-forward over 88 dates '
        ...
        + 'and empirical_spread_outcomes keep writing every pass -- so the forward '
        + 'record continues and the re-enable criterion has data to be judged on. The '
        + 'card keeps the cell label, cell n, the live/locked axis badge and the posted '
        + 'runline prices. The CLI (scripts/empirical-spread-edge.js) still prints the '
        + 'pp figures behind a banner, because measuring a broken number requires being '
        + 'able to see it.\n'
