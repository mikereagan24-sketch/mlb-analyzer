...
const GATES = [
  ...
  { id: 'bullpen_pool_lastname_fallback', key: null, on_expected: null,
    ...
    note: 'THE CODE. db/schema.js q.getBullpenWoba, roster filter: '
        ...
        + 'ROSTER FRESHNESS: CHECKED 2026-08-30, AND THE ROSTER IS NOT THE PROBLEM. '
        + 'All 784 team_rosters rows carry one identical updated_at (2026-08-29 '
        + '21:19:49) -- a full-table rewrite 23.8h old, and the table is already '
        + 'watched by utils/pipeline-freshness.js with a per-row lag sub-check, so '
        + 'there is no unmonitored freshness gap here. '
        + 'DISCRIMINATOR: pitcher_game_log is complete and continuous (177 dates, '
        + '2026-03-03 to 2026-08-29), so appearances are a valid test of who is '
        + 'actually on a team. 20 of the 22 have NEVER pitched for the team that '
        + 'admitted them. The remaining 2 last pitched 2026-04-07 (Shane Smith) and '
        + '2026-03-15 (Grant Rogers) -- months ago, correctly absent from a current '
        + 'roster. '
        + 'VERDICT: THE MATCH IS WRONG, in all 22 cases. Fix the surname filter. '
        + 'The earlier stale-roster hypothesis is REFUTED and should not be re-raised. '
        + 'TWO THINGS THAT MAKE SHANE SMITH WORSE THAN A NAME COLLISION: (a) all five '
        + 'of his logged outings are was_starter=1, so he is a STARTER and would not '
        + 'belong in a bullpen pool even if he were rostered; (b) his actuals sample '
        + '(338/278 BF) is 7.33x his logged 84 BF, where every current pitcher checked '
        + 'runs 0.6-1.2x -- so the actuals attached to him are not this season CWS '
        + 'data, and they dominate the actuals-heavy 0.25/0.75 bullpen blend. '
        + 'MEASURED COST TODAY: removing him moves CWS team bullpen wOBA by only '
        + '-0.0006, but the handedness splits move +0.0028 vsLHB and -0.0033 vsRHB '
        + 'and partly cancel in the blend. Small, not zero, and it is one phantom -- '
        + 'the other 21 were held out by the pool-selection rule, which is luck. '
        + 'AUDIT: scripts/audit-lastname-fallback-roster.js, re-runnable. '
