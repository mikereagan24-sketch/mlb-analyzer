...
const GATES = [
  ...
  { id: 'spread_edge_display_enabled', key: null, on_expected: false,
    criterion: 'OFF 2026-09-11. The empirical-spread pp figures are not shown on the '
             ...
             + 'RE-ENABLE CRITERION: a RECALIBRATED engine must beat the market out of '
             + 'sample on a FORWARD window -- n >= 2,000 distinct plays (one row per '
             + 'date/game/team/line/side, NOT per odds pass), n-weighted mean |bin '
             + 'error| strictly below the market\'s on the same window, fitted on data '
             + 'strictly before that window. Beating the RAW engine is not the bar; the '
             + 'raw engine is the thing that failed. Re-run: the walk-forward harness '
             + 'described in the founding measurement below.',
    criterion_type: 'calibration', precondition: null,
