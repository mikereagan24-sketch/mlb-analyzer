...
function getSettings() {
  ...
  return {
    RUN_MULT: s.run_mult || 48, HFA_BOOST: s.hfa_boost || 0.02,
    FAV_ADJ: s.fav_adj || 10, DOG_ADJ: s.dog_adj || 5,
    W_PIT: s.w_pit || 0.5, W_BAT: s.w_bat || 0.5,
    W_PROJ: s.w_proj || 0.65, W_ACT: s.w_act || 0.35,
    ML_VALUE_EDGE: s.ml_value_edge || 0.05, ML_LEAN_EDGE: s.ml_lean_edge || 0.02,
    TOT_VALUE_EDGE: s.tot_value_edge || 0.4, TOT_LEAN_EDGE: s.tot_lean_edge || 0.2,
  };
}
function getWobaIndex() {
  ...
  for (const r of rows) {
    ...
    const key = r.player_name.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-z\s]/g,'').replace(/\s+/g,' ').trim();
