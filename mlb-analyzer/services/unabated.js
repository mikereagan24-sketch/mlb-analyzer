...
const TOTAL_SOURCES = ['9','59','35','17','105','102','91','19','3'];
...
async function fetchUnabatedOdds(dateStr) {
  ...
  for (const {g,away,home} of Object.values(byMatchup)) {
    ...
    if (away==='kc' && home==='nyy') {
      const bt3dump = {};
      Object.entries(lines).forEach(([k,v])=>{ if(v.bt3!=null) bt3dump[k]={pts:v.bt3.points,price:v.bt3.americanPrice}; });
    ...
    const bySource = {};      // for ML Ã¢ÂÂ an0 only
    const bySourceTot = {};   // for totals Ã¢ÂÂ all an values, prefer bt3 with real points
    Object.entries(lines).forEach(([key,val])=>{
      ...
      // Totals: all an values Ã¢ÂÂ keep entry with real points (non-null, non-negative)
