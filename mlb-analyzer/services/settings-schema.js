...
const SETTINGS_SCHEMA = {
  ...
  signal_edge_hard_cap_pp: { type: 'number', min: 0.05, max: 0.50, default: 0.25,
    ...
    invariantMsg: 'signal_edge_hard_cap_pp must be > signal_edge_soft_cap_pp',
    help: 'DATA-INTEGRITY CEILING. Fractional probability-points (0.25 = 25pp). Signals with edge >= hard cap are suppressed from bet_signals + logged to bet_signal_audit. This is a corrupt-input trap, NOT an ROI filter: its job is to stop a garbage market line from becoming a bet, and on clean data it should never fire. Set back to 0.25 on 2026-08-22 after PR #174 repurposed it to 0.08 as a behavioral filter on ROI evidence that does not support that level -- 0.08 ranked 13th of 14 levels tested, suppressed the only positive edge band, and no level cleared the Tier 2 standard. The 0.25 level is empirical: of 1283 distinct signals the cap has suppressed in production, 279 carried an implausible market line (|ML| > 1000, up to +94400) and ALL 279 had edge >= 0.25, while ZERO signals below 25pp carried one. 25pp is where the corrupt population starts. Nothing upstream blocks these -- checkOddsSanity only FLAGS extreme lines and only when a cross-check book exists, so this cap is the last line of defence. Do not lower it to chase ROI. See docs/cap-as-sanity-bound-2026-08-22.md. Must be > signal_edge_soft_cap_pp.' },
