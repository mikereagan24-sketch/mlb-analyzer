...
Fix is in **#435** (`strGroup:'career'` + duplicate rejection). This doc
exists because the fix changes inputs, and a changed input invalidates
measurements taken before it — CLAUDE.md §"Re-check a deprioritizing
...
## Not verified, and it blocks #435

**Tidwell vs RHB has no row in the career pull.** Stored has him at 86 BF;
the 421-row career response for split 6 does not contain him. So the
reported target (146 BF / ~.282) is **unconfirmed** — not contradicted,
absent.

He is not alone. The career pull returns **fewer players than stored on
every key**, and actuals rows are bare names (no team-suffix expansion),
so stored rows *are* distinct players and the gap is real:

| data_key | stored players | career players | lost |
|---|---|---|---|
| `pit-act-rhb` | 539 | 421 | **118 (22%)** |
| `pit-act-lhb` | 492 | 466 | 26 (5%) |
| `bat-act-rhp` | 418 | 332 | **86 (21%)** |
| `bat-act-lhp` | 409 | 337 | 72 (18%) |

Every one of the 421 career rows matched a stored row, so the career set
is a strict **subset**. Cause unknown — plausibly a different default
qualifier on the aggregate grouping. Not chased further because FG
started returning 403 under repeated probing and it is not our service to
hammer.

**This is a trade, not a clean win.** #435 as written swaps a halved
sample for *no sample* on about one player in five, and a player with no
actuals row falls to pure projection. That needs resolving before #435
merges — either by confirming the dropped players are genuinely
unqualified, or by keeping the `'season'` pull and summing on our side
(the brief's fallback), which loses nobody.
