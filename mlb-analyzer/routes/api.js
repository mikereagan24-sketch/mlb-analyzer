...
// ─── Admin: diagnose FG actuals PA-qualifier threshold ──────────────────
// Compares FanGraphs' splits-leaders response with strAutoPt='true' vs
// 'false' on a single split (pit-act-rhb, splitCode=6) and reports row
// count delta, sample pitcher names, and how many rows have TBF≥100.
// Read-from-FG-and-report only — does NOT write to woba_data, does NOT
// modify fetchActualSplit, does NOT touch any other table. Inlined the
// fetch body here so the diagnostic stays self-contained.
//
// Single split tested: if strAutoPt's behavior is consistent on one
// pitcher-side fetch, it's consistent on all four actuals fetches.
// Testing all four would 4x the FG load for no marginal info.
//
// Auth reuses DB_DOWNLOAD_TOKEN via X-Admin-Token (same pattern as
// /admin/download-db). Same 401/503 semantics.
router.get('/admin/diagnose-fg-actuals-threshold', async (req, res) => {
  const stamp = new Date().toISOString();
  const ip = req.ip || req.connection?.remoteAddress || 'unknown';
  const expected = process.env.DB_DOWNLOAD_TOKEN;
  if (!expected) {
    console.log('[diagnose-fg] ' + stamp + ' ip=' + ip + ' result=missing-token-config');
  ...
  if (!ok) {
    console.log('[diagnose-fg] ' + stamp + ' ip=' + ip + ' result=auth-fail');
    return res.status(401).json({ error: 'unauthorized' });
  }
  ...
  function buildBody(strAutoPt) {
    return {
      strSplitArr: [6],
      strGroup: 'season',
      strPosition: 'P',
      strType: '2',
      strStartDate: iso(startDate),
      strEndDate: iso(endDate),
      strSplitTeams: false,
      dctFilters: [],
      strStatType: 'player',
      strAutoPt,
  ...
  async function probe(strAutoPt) {
    const r = await fetch(ACTUAL_URL, {
      method: 'POST',
      headers: fgHeaders,
      body: JSON.stringify(buildBody(strAutoPt)),
    });
    if (!r.ok) throw new Error('FG HTTP ' + r.status + ' for strAutoPt=' + strAutoPt);
    const json = await r.json();
    if (!json || !Array.isArray(json.data)) {
      throw new Error('No data array for strAutoPt=' + strAutoPt + '. Top keys: ' + Object.keys(json || {}).join(','));
    }
    const rows = json.data;
    const sample_pitcher_names = rows.slice(0, 5).map(rr => rr.PlayerName || rr.Name || '<unknown>');
    const count_with_sample_over_100 = rows.filter(rr => Number(rr.TBF || 0) >= 100).length;
    return { row_count: rows.length, sample_pitcher_names, count_with_sample_over_100 };
  }

  try {
    const withTrue = await probe('true');
    // 2-second delay between calls to respect FG's rate limiter.
    await new Promise(r => setTimeout(r, 2000));
    const withFalse = await probe('false');
    const result = {
      split: 'pit-act-rhb (splitCode=6, position=P)',
      with_strAutoPt_true: withTrue,
      with_strAutoPt_false: withFalse,
      delta: {
        row_count_diff: withFalse.row_count - withTrue.row_count,
        count_over_100_diff: withFalse.count_with_sample_over_100 - withTrue.count_with_sample_over_100,
      },
    };
    console.log('[diagnose-fg] ' + stamp + ' ip=' + ip + ' result=success rows_true=' + withTrue.row_count + ' rows_false=' + withFalse.row_count + ' delta=' + result.delta.row_count_diff);
    return res.json(result);
  } catch (err) {
    console.error('[diagnose-fg] ' + stamp + ' ip=' + ip + ' result=error msg=' + err.message);
