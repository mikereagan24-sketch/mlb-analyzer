...
router.get('/debug/kalshi', async (req, res) => {
  try {
    ...
    // Probe candidate game-level MLB series tickers
    const candidates = [
      'KXMLBGW','KXMLBGAME','KXMLBMW','KXMLBGWINNER',
      'KXMLBF5SPREAD','KXMLBTOTAL','KXMLBSPREAD','KXMLBRUNLINE',
      'KXMLBF5','KXMLBF5TOTAL','KXMLBF5WIN'
    ];
    const probe = {};
    for (const t of candidates) {
      const u = BASE+'/events?series_ticker='+t+'&status=open&limit=3';
      const d = await fetch(u, HDR).then(r=>r.json()).catch(()=>({events:[]}));
      probe[t] = {count:(d.events||[]).length, sample:(d.events||[]).slice(0,2).map(e=>({
        ticker:e.event_ticker, title:e.title, sub:e.sub_title
      }))};
    }

    // Also search ALL open markets for "baseball" or "MLB" title keywords
    const searchUrl = BASE+'/markets?status=open&limit=200';
    const searchData = await fetch(searchUrl, HDR).then(r=>r.json()).catch(()=>({markets:[]}));
    const mlbMarkets = (searchData.markets||[]).filter(m=>
      (m.title||'').match(/(mlb|baseball|cubs|dodgers|yankees|red sox|braves)/i) ||
      (m.ticker||'').includes('MLB') || (m.ticker||'').includes('KXMLB')
    ).slice(0,20).map(m=>({
      ticker:m.ticker, title:m.title, event_ticker:m.event_ticker,
      yes_bid:m.yes_bid_dollars, no_bid:m.no_bid_dollars, vol:m.volume_fp
    }));

    res.json({probe, mlbMarkets, total_markets: searchData.markets?.length});
